...
(defun +mail/buffer-face-mode-notmuch-search ()
  "Sets a fixed width (monospace) font in current buffer"
  (interactive)
  (let ((buffer-face-mode-face '(:family "Input Mono Narrow")))
    (buffer-face-mode)
...
(defun +mail/notmuch-update ()
  ...
  (start-process-shell-command
   "notmuch update"
   nil
   "sh ~/.local/bin/msmtp-runqueue.sh && afew -m && mbsync -qqa && notmuch new && afew -nt"))
...
;;;###autoload
(defun +mail/notmuch-overwrite-sender (o query-string &optional sender reply-all)
  "Overwrite sender"
  (let ((sender (concat user-full-name " <" user-mail-address ">")))
